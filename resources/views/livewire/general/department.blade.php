<div class="container">
    ...
    <div class="row mb-2">
        ...
        <div class="col-md-6">
            <button wire:click="create" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modal">Add Department</button>
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-1">
                        ...
                        <select wire:model="department_head" id="department_head" class="form-control mt-1">
                            <option value="">Select Employee</option>
                            @foreach($Employees as $Employees)
                            <option value="{{ $Employees->id }}">{{ $Employees->name }}</option>
                            @endforeach
                        </select>
                        @error('department_head') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="form-group mb-1">
                        ...
                        <input type="text" wire:model="phone_number" id="phone_number" type="number" class="form-control mt-1" placeholder="Enter phone ...">
                        @error('phone_number') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group mb-1">
                        <label for="phone_number">Email</label>
                        <input type="text" wire:model="email" id="email" type="email"class="form-control mt-1" placeholder="Enter email...">
        ...
        <table class="table table-bordered mt-5">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ ($department->department_head) ? $department->Department_Head->name : 'Unknown' }}</td>

                    <td>{{ $department->location  ?? 'N/A' }}</td>
                    <td>{{ $department->phone_number  ?? 'N/A' }}</td>
                    ...
                    <td>{{ ($department->created_by) ? $department->Created_By->name : 'Unknown' }}</td>

                    <!-- Action buttons for editing and deleting -->
                    <td>
                        <button wire:click="edit({{ $department->id }})" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modal" title="Edit">Edit</button>
                        <button wire:click="delete({{ $department->id }})" class="btn btn-danger btn-sm" title="Delete">Delete</button>
                    </td>
                </tr>
                @endforeach
            </tbody>
